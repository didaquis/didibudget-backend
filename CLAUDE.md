...
**GraphQL API** (Apollo Server 5 / `@apollo/server` on Express, wired via `@as-integrations/express5`) backed by **MongoDB via Mongoose**. Entry point is [src/server.ts](src/server.ts).
